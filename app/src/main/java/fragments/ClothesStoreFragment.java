...
public class ClothesStoreFragment extends Fragment {
    ...
    private void init(View view) {
        ...
        mRestaurantRecyclerView = view.findViewById(R.id.restaurant_recyclerView3);
        mRestaurantRecyclerView.setLayoutManager(linearLayoutManager);
        cuisineGridView3 = view.findViewById(R.id.cuisineGridView3);
        cuisineGridView3.setLayoutManager(linearLayoutManager2);
    ...
    private void getRestaurantDetails() {
        ...
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                ...
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, ClothesStoreFragment.RestaurantItemViewHolder> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, ClothesStoreFragment.RestaurantItemViewHolder>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull ClothesStoreFragment.RestaurantItemViewHolder holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            holder.itemView.setOnClickListener(view -> {
                                ...
                                Intent intent = new Intent(requireContext(), MainRestaurantPageActivity.class);
            ...
            @NonNull
            @Override
            public ClothesStoreFragment.RestaurantItemViewHolder onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.restaurant_main_detail, group, false);
                return new ClothesStoreFragment.RestaurantItemViewHolder(view);
            }
    ...
    private void getRestaurantDetails2() {
        Query query = db.collection("[Clothes]").whereEqualTo("top","Yes");
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                .setQuery(query, RestaurantDetail.class)
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, ClothesStoreFragment.RestaurantItemViewHolder> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, ClothesStoreFragment.RestaurantItemViewHolder>(menuItemModel) {
        ...
        restaurantAdapter.startListening();
        SnapHelper helper = new LinearSnapHelper();
        helper.attachToRecyclerView(cuisineGridView3);
        restaurantAdapter.notifyDataSetChanged();
        cuisineGridView3.setAdapter(restaurantAdapter);
    }
