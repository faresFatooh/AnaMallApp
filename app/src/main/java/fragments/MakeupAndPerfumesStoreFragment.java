...
public class MakeupAndPerfumesStoreFragment extends Fragment {
    ...
    public MakeupAndPerfumesStoreFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view =   inflater.inflate(R.layout.fragment_makeup_and_perfumes_store, container, false);
        init(view);
...
//            startActivity(intent);
        ...
        mRestaurantRecyclerView = view.findViewById(R.id.restaurant_recyclerView5);
        mRestaurantRecyclerView.setLayoutManager(linearLayoutManager);
        linearLayoutManager2 = new GridLayoutManager(getContext(), 2);
        mRestaurantRecyclerView2 = view.findViewById(R.id.cuisineGridView5);
    ...
    private void getRestaurantDetails(Context context) {
        Query query = db.collection("[MakeupAndPerfumes]");
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                ...
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
            ...
            @NonNull
            @Override
            public MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.restaurant_main_detail, group, false);
                return new MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder(view);
            }
    ...
    private void getRestaurantDetails2(Context context) {
        Query query = db.collection("[MakeupAndPerfumes]").whereEqualTo("top","Yes");
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                .setQuery(query, RestaurantDetail.class)
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder2> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder2>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder2 holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
            ...
            @NonNull
            @Override
            public MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder2 onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.custom_cuisine_layout, group, false);
                return new MakeupAndPerfumesStoreFragment.RestaurantItemViewHolder2(view);
            }
