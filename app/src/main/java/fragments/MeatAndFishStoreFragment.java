...
public class MeatAndFishStoreFragment extends Fragment {
    ...
    public MeatAndFishStoreFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_meat_and_fish_store, container, false);
        init(view);
...
//            startActivity(intent);
        ...
        mRestaurantRecyclerView = view.findViewById(R.id.restaurant_recyclerView6);
        ...
        mRestaurantRecyclerView2 = view.findViewById(R.id.cuisineGridView6);
    ...
    private void getRestaurantDetails(Context context) {
        Query query = db.collection("[MeatAndFish]");
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                ...
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, MeatAndFishStoreFragment.RestaurantItemViewHolder> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, MeatAndFishStoreFragment.RestaurantItemViewHolder>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull MeatAndFishStoreFragment.RestaurantItemViewHolder holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
            ...
            @NonNull
            @Override
            public MeatAndFishStoreFragment.RestaurantItemViewHolder onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.restaurant_main_detail, group, false);
                return new MeatAndFishStoreFragment.RestaurantItemViewHolder(view);
            }
    ...
    private void getRestaurantDetails2(Context context) {
        Query query = db.collection("[MeatAndFish]").whereEqualTo("top", "Yes");
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                .setQuery(query, RestaurantDetail.class)
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, MeatAndFishStoreFragment.RestaurantItemViewHolder2> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, MeatAndFishStoreFragment.RestaurantItemViewHolder2>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull MeatAndFishStoreFragment.RestaurantItemViewHolder2 holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
            ...
            @NonNull
            @Override
            public MeatAndFishStoreFragment.RestaurantItemViewHolder2 onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.custom_cuisine_layout, group, false);
                return new MeatAndFishStoreFragment.RestaurantItemViewHolder2(view);
            }
