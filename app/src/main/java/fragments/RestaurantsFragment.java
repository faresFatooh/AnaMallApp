...
//            startActivity(intent);
    ...
    private void getRestaurantDetails(Context context) {
        ...
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                ...
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, RestaurantItemViewHolder> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, RestaurantsFragment.RestaurantItemViewHolder>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull RestaurantsFragment.RestaurantItemViewHolder holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            Glide.with(context).load(model.getRestaurant_spotimage()).into(holder.mRestaurantSpotImage);
            ...
            @NonNull
            @Override
            public RestaurantsFragment.RestaurantItemViewHolder onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.restaurant_main_detail, group, false);
                return new RestaurantsFragment.RestaurantItemViewHolder(view);
            }
...
//        helper.attachToRecyclerView(mRestaurantRecyclerView);
    ...
    private void getRestaurantDetails2(Context context) {
        ...
        FirestoreRecyclerOptions<RestaurantDetail> menuItemModel = new FirestoreRecyclerOptions.Builder<RestaurantDetail>()
                .setQuery(query, RestaurantDetail.class)
                .build();
        FirestoreRecyclerAdapter<RestaurantDetail, RestaurantItemViewHolder2> restaurantAdapter = new FirestoreRecyclerAdapter<RestaurantDetail, RestaurantsFragment.RestaurantItemViewHolder2>(menuItemModel) {
            @SuppressLint("SetTextI18n")
            @Override
            public void onBindViewHolder(@NonNull RestaurantsFragment.RestaurantItemViewHolder2 holder, int position, @NonNull RestaurantDetail model) {
                if (mCurrentUser != null) {
                    DocumentReference docRef = db.collection("UserList").document(mCurrentUser.getUid());
                    docRef.get().addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
            ...
            @NonNull
            @Override
            public RestaurantsFragment.RestaurantItemViewHolder2 onCreateViewHolder(@NonNull ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.custom_cuisine_layout, group, false);
                return new RestaurantsFragment.RestaurantItemViewHolder2(view);
            }
